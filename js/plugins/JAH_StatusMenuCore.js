 ...
 * Creates an advanced Status window with multiple pages for each Actor.  Supports profiles and portraits.
 * @author JahwsUF
 *
 ...
 * @param Page 2 Window
 * @desc Window corresponding to the first status page.
 * Default: JAH_Window_Elemental_Resistances
 * @default JAH_Window_Elemental_Resistances
 *
 * @param ---Page 3---
 * Default: 
 ...
 * @default Elemental Damage Received
 *
 * @param Ele. Col Txt Width
 * @desc The amount of space to give each state's name on the state page.
 * @default 160
 *
 * @param Ele. Col Val Width
 * @desc The amount of space to give each state's value / classification on the state page.
 * @default 80
 *
 ...
 * @default State Resistance
 *
 * @param State Col Txt Width
 * @desc The amount of space to give each state's name on the state page.
 * @default 160
 *
 * @param State Col Val Width
 * @desc The amount of space to give each state's value / classification on the state page.
 * @default 80
 *
 ...
 * x = 1 provides the default scaling behavior; x = 0.5 halves the size, while
...
//
// The objects defined here (before the plugin parameters are loaded) are all
// used to specify the layout and display patterns of the parameter-oriented 
// window classes used within the Status menu.
//
...
JAH_StatusMenuCore_ParamEntry.prototype.draw = function(window, x, y, columnPadding) {
...
JAH_StatusMenuCore_ResistEntry.prototype.initialize = function(symbol, name, asPercent, txtWidth, valWidth, align) {
	...
	{
		if(symbol == "pdr" || symbol == "mdr" || symbol.indexOf("elementRate") > -1)
		{
...
JAH_StatusMenuCore_ResistEntry.prototype.initName = function() {
	...
	if(name === undefined || name.trim() == "")
	{
		...
		else name = "undefined";  // includes if they don't name pdr or mdr.
	}
...
JAH_StatusMenuCore_ResistEntry.prototype.categorizePercentValue = function(val) {
	...
	for(var i = 0; i < JahwsUF.StatusMenuCore.Constants.MaxCategories; i++)
	{
...
JAH_StatusMenuCore_ResistEntry.prototype.draw = function(window, x, y, columnPadding) {
...
//
// Stores a text header for insertion within a parameter page,
// allowing custom text in the page's body.
 
function JAH_StatusMenuCore_HeaderEntry() {
	this.initialize.apply(this, arguments);
}
 
JAH_StatusMenuCore_HeaderEntry.prototype.constructor = JAH_StatusMenuCore_HeaderEntry;

JAH_StatusMenuCore_HeaderEntry.prototype.initialize = function(text, colsToMerge, page, align) {
	this._text = text;
	this._knownWidth = 0;
	
	var colData = page.Columns;

	for(var c = 0; c < colsToMerge.length; c++)
	{
		var col = colsToMerge[c];
		this._knownWidth += colData[col].TextWidth + colData[col].ValueWidth;  // absorb the standard column entry space.
	}
	
	// How many EXTRA columns did we absorb? (Don't count the first in padding calcs.)
	this._colsToMerge = colsToMerge.length - 1;
	this._align = align || 'center';
}; 

JAH_StatusMenuCore_HeaderEntry.prototype.draw = function(window, x, y, columnPadding) {
	
	if(!this._text) return;
	
	window.changeTextColor(window.systemColor());
	
	var width = this._knownWidth + columnPadding * this._colsToMerge;
	
	window.drawText(this._text, x, y, width, this._align);
	window.resetTextColor();
}; 

// -------------------------------------------------------
// JAH_StatusMenuCore_BlankEntry
//
// A completely empty entry in the parameter page.  Generally used when a 
// HeaderEntry to the left absorbs the position this one fills, to keep the buffer.
 
function JAH_StatusMenuCore_BlankEntry() {
	this.initialize.apply(this, arguments);
}
 
JAH_StatusMenuCore_BlankEntry.prototype.constructor = JAH_StatusMenuCore_BlankEntry;

JAH_StatusMenuCore_BlankEntry.prototype.initialize = function() {
}; 

JAH_StatusMenuCore_BlankEntry.prototype.draw = function(window, x, y, columnPadding) {
	return;
}; 
...
JahwsUF.StatusMenuCore.Constants = {};
JahwsUF.StatusMenuCore.Constants.MaxPageColumns   = 3;
JahwsUF.StatusMenuCore.Constants.MaxPageCount     = 6;
JahwsUF.StatusMenuCore.Constants.MaxPageGroupings = 10;
JahwsUF.StatusMenuCore.Constants.MaxCategories    = 8;
JahwsUF.StatusMenuCore.Constants.MaxPageEntries   = 48;
...
JahwsUF.StatusMenuCore.Param.ParamPages[0].ColWidths     = [];
...
JahwsUF.StatusMenuCore.Param.ResistPages                           = {};
JahwsUF.StatusMenuCore.Param.ResistPages.Elements                  = {};
JahwsUF.StatusMenuCore.Param.ResistPages.Elements.Columns          = [];
JahwsUF.StatusMenuCore.Param.ResistPages.Elements.ColWidths        = []; 
JahwsUF.StatusMenuCore.Param.ResistPages.Elements.RebalanceColumns = false;
JahwsUF.StatusMenuCore.Param.ResistPages.Elements.DisplayEquips    = false;
JahwsUF.StatusMenuCore.Param.ResistPages.States                    = {};
JahwsUF.StatusMenuCore.Param.ResistPages.States.Columns            = [];
JahwsUF.StatusMenuCore.Param.ResistPages.States.RebalanceColumns   = {};
JahwsUF.StatusMenuCore.Param.ResistPages.States.ColWidths          = [];
JahwsUF.StatusMenuCore.Param.ResistPages.States.DisplayEquips      = false;
...
for (var i = 1; i <= JahwsUF.StatusMenuCore.Constants.MaxPageCount; i++) {
...
// Step 1:  Read in the param column formatting data.  
// 1)  How many entries per column?
// 2)  How many columns on the page?
// 3)  How wide should the columns be?
...
{
	var line = "Number(JahwsUF.StatusMenuCore.Parameters['Params in Col " + i + "'])";
	var count = eval(line);
	...
	// Holds all displayables for the column.  In particular, pre-computed objects
	// corresponding to the game designer's desired page specification.
	//
	// This is done here at load time for optimization and reduction of windowing code
	// complexity.  (Object orientation = good.)
	//
	// Admittedly, I don't pre-analyze the final positions based on resolution, in case
	// someone eventually makes a dynamically resolution-changing plugin.
	
	column.Entries = [];   // An EXTREMELY important field.
...
// Display equipment options.  (It's a part of the makeup of a page.)
...
var pdr_mdr_asResists = eval(JahwsUF.StatusMenuCore.Parameters['pdr/mdr as Resists']);

if(pdr_mdr_asResists != true)
	pdr_mdr_asResists = false;
	
var rebalance = eval(JahwsUF.StatusMenuCore.Parameters['Rebalance Emptiness']);
...
// Read in parameter grouping title spec data.  Construct their page entries now so that
// we can detect what page positions are occupied and make the necessary adjustments.
...
{
	GroupingTitles[i] = JahwsUF.StatusMenuCore.Parameters['Grouping Title ' + i];
	
	GroupingRows[i] = Number(JahwsUF.StatusMenuCore.Parameters['Row Placement ' + i]);
	if(!GroupingRows[i])
		GroupingRows[i] = undefined;
	else GroupingRows[i]--;
	
	var colSpec = JahwsUF.StatusMenuCore.Parameters['Group Columns ' + i ];
	...
	// If we actually are defining a header grouping.
...
// If a slot is occupied, map all parameters previously positioned here or below down an extra row.
// (can occur multiple times)

var EntryMapping = [];  // This 2D array will hold the positions at which parameters ought be placed
                        // within each column.
...
{
	EntryMapping[c] = [];  // [column][row]
	var row = 0;
	
	for(var r = 0; row < pageSpecs.Columns[c].Size; r++)
	{
		// All headers and intentional blanks are already defined.
		// Parameters are not, so we find empty spaces and make note of
		// which parameter in the column gets mapped to each.
...
// Resistance alignment formatting data.  Awkwardly read here because its data needs 
// to be passed into any resistance-oriented Param page entry structures when initialized.

// Only has bearing on the alignment of text within its page entry.  Does not affect the
// actual position of entries.
...
for (var columnPlacement = 0; columnPlacement < JahwsUF.StatusMenuCore.Param.ParamPages[paramPageId].Columns.length; columnPlacement++) {
	...
	for (var row = 0; row < currentColumn.Size; row++) {
		...
		// The precomputed position for this parameter.  
		// Ex:  EntryMapping[0][1] maps the first column's second parameter to its actual line number,
		// thus effectively to its eventual y-value when actually displayed in JAH_Window_Status_Params.
		//
		// (EntryMapping, as with many of my other arrays, are zero-based.)
...
var eleTextWidth = Number(JahwsUF.StatusMenuCore.Parameters['Ele. Col Txt Width']);
if(eleTextWidth == '' || eleTextWidth == undefined || isNaN(eleTextWidth))
{
	eleTextWidth = 160;
}

var eleValWidth = Number(JahwsUF.StatusMenuCore.Parameters['Ele. Col Val Width']);
if(eleValWidth == '' || eleValWidth == undefined || isNaN(eleValWidth))
{
	eleValWidth = 80;
}
...
for (var i = 1; i <= JahwsUF.StatusMenuCore.Constants.MaxPageColumns; i++)
{
	...
	{
		JahwsUF.StatusMenuCore.Param.ResistPages.Elements.Columns[i-1]            = {};
		JahwsUF.StatusMenuCore.Param.ResistPages.Elements.Columns[i-1].TextWidth  = eleTextWidth;
		JahwsUF.StatusMenuCore.Param.ResistPages.Elements.Columns[i-1].ValueWidth = eleValWidth;
		JahwsUF.StatusMenuCore.Param.ResistPages.Elements.Columns[i-1].Entries    = [];
		JahwsUF.StatusMenuCore.Param.ResistPages.Elements.Columns[i-1].Size       = eles;
...
// --------- Construct elemental resist page entry layout ------------
var eleHeader = JahwsUF.StatusMenuCore.Parameters['Ele. Resist Header'];

if(eleHeader == undefined)
	eleHeader == "";
	
if(typeof eleHeader == "string" && eleHeader.trim() == "")
	eleHeader = "";

var eleRowOffset = 0;
var elePage = JahwsUF.StatusMenuCore.Param.ResistPages.Elements;

if(eleHeader != "")
{
	eleRowOffset = 2; // We insert two lines for the text title.
	
	// Insert header into the page construction!
	var colCount = elePage.Columns.length;
	
	var mergeCols = [];
	for(var i = 0; i < colCount; i++)
		mergeCols.push(i);
	
	elePage.Columns[0].Entries[0] = new JAH_StatusMenuCore_HeaderEntry(eleHeader, mergeCols, elePage);
	elePage.Columns[0].Entries[1] = new JAH_StatusMenuCore_HeaderEntry("", mergeCols, elePage);

	elePage.ColWidths[0] = colCount;
	elePage.ColWidths[1] = colCount;
		
	for(var i = 1; i < colCount; i++)
	{
		elePage.Columns[i].Entries[0] = new JAH_StatusMenuCore_BlankEntry();
		elePage.Columns[i].Entries[1] = new JAH_StatusMenuCore_BlankEntry();
	}
}

var i = 1;

for (var columnPlacement = 0; columnPlacement < elePage.Columns.length; columnPlacement++) {

	var currentColumn = elePage.Columns[columnPlacement];

	for (var row = 0; row < currentColumn.Size; row++) {
		
		while(elementsToOmit.indexOf(i) > -1) 
		{
			i++;
		}
		
		var rowPlacement = row + eleRowOffset;
	  
		currentColumn.Entries[rowPlacement] = new JAH_StatusMenuCore_ResistEntry("elementRate(" + i + ")", undefined, undefined, currentColumn.TextWidth, currentColumn.ValueWidth, JahwsUF.StatusMenuCore.Param.CenterEleValues ? 'center' : 'right');
		elePage.ColWidths[rowPlacement] = (elePage.ColWidths[rowPlacement] || 0) + 1;
		
		i++;
	}
}

//JahwsUF.StatusMenuCore.Param.EleResistsHeader = eleHeader;
...
var stateTextWidth = Number(JahwsUF.StatusMenuCore.Parameters['State Col Txt Width']);
if(stateTextWidth == '' || stateTextWidth == undefined || isNaN(stateTextWidth))
{
	stateTextWidth = 160;
}

var stateValWidth = Number(JahwsUF.StatusMenuCore.Parameters['State Col Val Width']);
if(stateValWidth == '' || stateValWidth == undefined || isNaN(stateValWidth))
{
	stateValWidth = 80;
}
...
for (var i = 1; i <= JahwsUF.StatusMenuCore.Constants.MaxPageColumns; i++)
{
	...
	var states = eval(line);
	//JahwsUF.StatusMenuCore.Param.StateResistsInColumn[i-1] = state;
	
	if(states == 0 || states == '' || isNaN(states))
	{
		break;
	}
	else
	{
		JahwsUF.StatusMenuCore.Param.ResistPages.States.Columns[i-1]            = {};
		JahwsUF.StatusMenuCore.Param.ResistPages.States.Columns[i-1].TextWidth  = stateTextWidth;
		JahwsUF.StatusMenuCore.Param.ResistPages.States.Columns[i-1].ValueWidth = stateValWidth;
		JahwsUF.StatusMenuCore.Param.ResistPages.States.Columns[i-1].Entries    = [];
		JahwsUF.StatusMenuCore.Param.ResistPages.States.Columns[i-1].Size       = states;
		resistanceColumnCount = i;
	}
}
...
// --------- Construct elemental resist page entry layout ------------
var stateHeader = JahwsUF.StatusMenuCore.Parameters['State Resist Header'];

if(stateHeader == undefined)
	stateHeader == "";
	
if(typeof stateHeader == "string" && stateHeader.trim() == "")
	stateHeader = "";

var stateRowOffset = 0;
var statePage = JahwsUF.StatusMenuCore.Param.ResistPages.States;

if(stateHeader != "")
{
	stateRowOffset = 2; // We insert two lines for the text title.
	
	// Insert header into the page construction!
	var colCount = statePage.Columns.length;
	
	var mergeCols = [];
	for(var i = 0; i < colCount; i++)
		mergeCols.push(i);
	
	statePage.Columns[0].Entries[0] = new JAH_StatusMenuCore_HeaderEntry(stateHeader, mergeCols, statePage);
	statePage.Columns[0].Entries[1] = new JAH_StatusMenuCore_HeaderEntry("", mergeCols, statePage);

	statePage.ColWidths[0] = colCount;
	statePage.ColWidths[1] = colCount;
		
	for(var i = 1; i < colCount; i++)
	{
		statePage.Columns[i].Entries[0] = new JAH_StatusMenuCore_BlankEntry();
		statePage.Columns[i].Entries[1] = new JAH_StatusMenuCore_BlankEntry();
	}
}

//JahwsUF.StatusMenuCore.Param.StateResistsHeader = stateHeader;

var i = 1;

for (var columnPlacement = 0; columnPlacement < statePage.Columns.length; columnPlacement++) {

	var currentColumn = statePage.Columns[columnPlacement];

	for (var row = 0; row < currentColumn.Size; row++) {
		
		while(statesToOmit.indexOf(i) > -1) 
		{
			i++;
		}
		
		var rowPlacement = row + stateRowOffset;
	  
		currentColumn.Entries[rowPlacement] = new JAH_StatusMenuCore_ResistEntry("stateRate(" + i + ")", undefined, undefined, currentColumn.TextWidth, currentColumn.ValueWidth, JahwsUF.StatusMenuCore.Param.CenterEleValues ? 'center' : 'right');
		statePage.ColWidths[rowPlacement] = (statePage.ColWidths[rowPlacement] || 0) + 1;
...
for (var i = 1; i <= JahwsUF.StatusMenuCore.Constants.MaxCategories; i++)
{
...
Scene_Status.prototype.create = function() {
	...
	{
		var window = JahwsUF.StatusMenuCore.Param.PageWindows[i];
		
		var constructEval = "new " + window;
		
		if(window == "JAH_Window_Status_Params")
		{
			constructEval = constructEval + "(" + paramPageId + ")";
			
			if(JahwsUF.StatusMenuCore.Param.ParamPages[paramPageId] === undefined)
				throw "Parameter page ID " + paramPageId + " has no specification - cannot create!";
			
			paramPageId++;
		}
		else if(window == "JAH_Window_Status_Resistances")  // For backward compatibility.
		{
			window = "JAH_Window_Elemental_Resistances";
			constructEval = "new " + window;
		}
		else constructEval = constructEval + "()";
		
		//JAH_Window_Status_Params
		
		var page = eval(constructEval);
		this.addWindow(page);
	...
	if(JahwsUF.StatusMenuCore.Param.TotalPages == 1)
	{
		// Hide the status submenu, reclaim its space for the lone window.
		// The submenu still exists and is active for input commands.
		this._statusWindow.hide();
		
		var page = this._pageWindows[0];
		
		// Forced resizing of the window.
		// The drawing contents are unfortunately not replaced on a resize,
		// so I force the resize in the second line below.
		page.move(0, 0, Graphics.boxWidth, Graphics.boxHeight);
		page.contents = new Bitmap(Graphics.boxWidth, Graphics.boxHeight);
	}
...
//
// In case it's not otherwise clear within the code, it is THIS WINDOW that
// actually receives all the input commands, not the displayed Status window page.
// 
// It forwards the resulting instructions to the other windows as appropriate.
...
JAH_Window_Status_Base.prototype.drawVanillaHeader = function(y, lineIndex) 
{
	...
	// drawBasicInfo and drawGauges are artifacts left over from the original RPG Maker MX
	// Status window.  I've restructured them to be sure, but I never got around to actually
	// removing them as separate functions.
...
// Actor picture, name, nickname, level, etc.
...
JAH_Window_Status_Params.prototype.initialize = function(pageId, pageParam) {
    JAH_Window_Status_Base.prototype.initialize.call(this);
	
	if(!pageId) pageId = 0;
	
	// After the constructor, this field is only used for error messages.
	this.pageId = pageId;
	
	if(!pageParam)
		this._pageParameterization = JahwsUF.StatusMenuCore.Param.ParamPages[pageId];
	else this._pageParameterization = pageParam;
};

// The Param page's master display method.  Does quite a fair bit of work.
JAH_Window_Status_Params.prototype.drawPageBlock = function(y, lineIndex) {
	...
	else if(eval(this._pageParameterization.DisplayEquips) == true)
	{
		console.log("Insufficient space to display the equipment listing on this Status window parameter page!  Page id = " + this.pageId);
	}
	
	// --------------------------- STEP 2 --------------------------------
	// Precompute all relevant column spacing data.
	
	// How much padding should be used when the given number of columns are in use?
	var EvenPaddings = [];
	
	// ColumnXCoords[totalcols][curCol];  // when there are a total of totalcols in use, curCol's x value is given here.
	var ColumnXCoords = [];
	
	for(var i = paramColumnsRequested; i >= 1; i--)
	{
		var neededWidth = 0;
		
		for(var j=0; j < i; j++)
		{
			neededWidth += this._pageParameterization.Columns[j].TextWidth + this._pageParameterization.Columns[j].ValueWidth;
		}
		neededWidth += 2 * i * this.standardPadding();
		var availWidth = availableWidth;
		var totalActivatedColumns = i;
				
		if(displayEquips && i == paramColumnsRequested)
		{
			neededWidth += 2 * this.standardPadding() + this._pageParameterization.EquipColumnWidth;
			totalActivatedColumns += 1;
		}
		else if(displayEquips)
		{
			availWidth -= 2 * EvenPaddings[paramColumnsRequested] + this._pageParameterization.EquipColumnWidth;
		}
		
		// Determine the actual spacings to be utilized.
		// We included the base this.standardPadding() amount in the req'd width... be sure to add it back!
		EvenPaddings[i] = (availWidth - neededWidth) / (2 * totalActivatedColumns) + this.standardPadding();
		
		// Determine the x-coords to be used for everything to be displayed.
		ColumnXCoords[i] = [];		
		ColumnXCoords[i][0] = EvenPaddings[i];  //Base position of first column.
		
		for(var c = 1; c <= i; c++)
		{
			ColumnXCoords[i][c] = ColumnXCoords[i][c-1] + this._pageParameterization.Columns[c - 1].TextWidth + this._pageParameterization.Columns[c - 1].ValueWidth + 2 * EvenPaddings[i];
		}
	}	
	
	// --------------------------- STEP 3 --------------------------------
	// Actually DRAW all the things.
		
	// Aethestics calculations now complete.  Time to actually display ALL the things!
    for (var column = 0; column < this._pageParameterization.Columns.length; column++) {
		
		var y2 = y;
		var l2 = lineIndex;
		
		for (var row = 0; row < this._pageParameterization.Columns[column].Entries.length; row++) {
		
			// Positioning of the entry.  
			var x = ColumnXCoords[paramColumnsRequested][column];
			var evenPaddingAmount = EvenPaddings[paramColumnsRequested];
		
			if(this._pageParameterization.RebalanceColumns)
			{
				// If this row HAS entries.  Some rows may be left blank intentionally,
				// in which case nothing will even be printed.
				if(this._pageParameterization.ColWidths[row])
				{
					x = ColumnXCoords[this._pageParameterization.ColWidths[row]][column];
					evenPaddingAmount = EvenPaddings[this._pageParameterization.ColWidths[row]];
				}
				
				// Determine the true column - what if there's a blank in a column to our left?
				for(var c2 = 0; c2 < column; c2++)
				{
					if(!this._pageParameterization.Columns[c2].Entries[row])
						// Erase the blank entry from existence, reclaiming its space.
						x -= this._pageParameterization.Columns[c2].TextWidth + this._pageParameterization.Columns[c2].ValueWidth + 2 * EvenPaddings[paramColumnsRequested];
				}
			}
			...
			if(!(paramEntry == undefined))
			{
				if(l2 >= 0)
				{
					paramEntry.draw(this, x, y2, 2 * evenPaddingAmount);
				}
			}
	...
	if(displayEquips)
		this.drawEquipments(ColumnXCoords[paramColumnsRequested][paramColumnsRequested], y + lineHeight * lineIndex);
};
...
JAH_Window_Status_Params.prototype.getLineCount = function(portraitMode) {
	...
	{
		if(maxCol < this._pageParameterization.Columns[i].Entries.length)
...
//-----------------------------------------------------------------------------
// JAH_Window_Elemental_Resistances
//
...
function JAH_Window_Elemental_Resistances() {
	this.initialize.apply(this, arguments);
}

//JAH_Window_Elemental_Resistances.prototype = Object.create(JAH_Window_Status_Base.prototype);
JAH_Window_Elemental_Resistances.prototype = Object.create(JAH_Window_Status_Params.prototype);
JAH_Window_Elemental_Resistances.prototype.constructor = JAH_Window_Elemental_Resistances;

JAH_Window_Elemental_Resistances.prototype.initialize = function() {
    //JAH_Window_Status_Base.prototype.initialize.call(this);
	JAH_Window_Status_Params.prototype.initialize.call(this, "Elemental Resistances", JahwsUF.StatusMenuCore.Param.ResistPages.Elements);
};
...
JAH_Window_State_Resistances.prototype = Object.create(JAH_Window_Status_Params.prototype);
...
JAH_Window_State_Resistances.prototype.initialize = function() {
    //JAH_Window_Status_Base.prototype.initialize.call(this);
	JAH_Window_Status_Params.prototype.initialize.call(this, "State Resistances", JahwsUF.StatusMenuCore.Param.ResistPages.States);
};
...
//
// Quite useful in enforcing an absolute margin on text to be displayed in the Profile
// window.
//
// This window is NEVER displayed; its contents are copied into the Profile window for
// display via the blt function.
//
...
// Implements a Status page that displays a single Actor's parameters.
//
// All text processing is actually handled by the Profile_Texter window and then
// copied internally via blt for display.
...
JAH_Window_Status_Profile.prototype.initialize = function() {
	...
	this.totalLineCount = undefined; // Stores pre-computed line count information; it's an expensive calculation when word-wrapping is enabled.
};
...
JAH_Window_Status_Profile.prototype.getLineCount = function() {
	...
	// Sadly, the base code doesn't actually update the bitmap used for drawing!
	...
	{
		// Compute the number of lines in the profile text.
		...
		totalHeight = Math.ceil(this.calcTextHeight.call(this.hiddenTextWindow, textState, true));
	}
...
JAH_Window_Status_Profile.prototype.printScrolledText = function(text, x, y, scroll) {
	...
	// Copy the hiddenTextWindow's text to our window.  We can set a limit on what areas are copied,
	// which allows for a text border to be enforced on profile text!
