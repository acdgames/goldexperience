 ...
 *
 * @param Rank1 Rarity
 * @desc The percentage to roll a Rank 1 Pull
 * @default -1
 * @require 0
 *
 * @param Rank2 Rarity
 * @desc The percentage to roll a Rank 2 Pull
 * @default -1
 * @require 0
 *
 * @param Rank3 Rarity
 * @desc The percentage to roll a Rank 3 Pull
 * @default -1
 * @require 0
 *
 * @param Rank4 Rarity
 * @desc The percentage to roll a Rank 4 Pull
 * @default -1
 * @require 0
 *
 * @param Rank5 Rarity
 * @desc The percentage to roll a Rank 5 Pull
 * @default -1
 * @require 0
 *
 * @param ME
 ...
 *   Gacha rateup item 1        # Multiply item #1 pull rate in Gacha by 5
 *   Gacha rateclear            # Clear all current Rate ups
 *
 *
 * Item Note:
...
(function () {
    ...
    var effect = Number(parameters['Effect'] || '131');
    var rankEffect = [];
	...
	var rank1rate = (Number(parameters['Rank1 Rarity'] || '0'));
	var rank2rate = (Number(parameters['Rank2 Rarity'] || '0'));
	var rank3rate = (Number(parameters['Rank3 Rarity'] || '0'));
	var rank4rate = (Number(parameters['Rank4 Rarity'] || '0'));
	var rank5rate = (Number(parameters['Rank5 Rarity'] || '0'));
    ...
    Game_Interpreter.prototype.pluginCommand = function (command, args) {
        ...
        if (command === "Gacha") {
            switch (args[0]) {
                ...
                case 'rateup':
                    $gameSystem.addRateUp(args[1], Number(args[2]));
                    break;

                case 'rateclear':
                    $gameSystem.clearRateUp();
                    break;
	...
	Game_System.prototype.addRateUp = function(type, dataId) {
	this.rateupItems = rateupItems || [];
	this.rateupWeapons = rateupWeapons || [];
	this.rateupArmors = rateupArmors || [];
        if (type == 'item') {
			this.rateupItems.push(dataId);
		} else if (type == 'weapon') {
			this.rateupWeapons.push(dataId);
		} else  {
			this.rateupArmors.push(dataId);
		}
    };
	
	Game_System.prototype.clearRateUp = function() {
        this.rateupItems.length = 0;
		this.rateupWeapons.length = 0;
		this.rateupArmors.length = 0;
    };
 ...
 Scene_Gacha.prototype.initialize = function () {
	 // Launcher
        ...
        for (i = 1; i < $dataItems.length; i++) {
            item = $dataItems[i];
            var rank = Number(item.meta.gachaRank || '-1');
            if ($gameSystem.isInGacha(item)) {
                numLot = Number(item.meta.gachaNumLot || '1');
				if ($gameSystem.rateupItems.contains(item.id)) {
					numLot *= 3;
				}
                for (j = 0; j < numLot; j++) {
                    this._lot.push(item);
        ...
        for (i = 1; i < $dataWeapons.length; i++) {
            ...
            if ($gameSystem.isInGacha(item)) {
                numLot = Number(item.meta.gachaNumLot || '1');
				if ($gameSystem.rateupWeapons.contains(item.id)) {
					numLot *= 3;
				}
                for (j = 0; j < numLot; j++) {
                    this._lot.push(item);
        ...
        for (i = 1; i < $dataArmors.length; i++) {
            ...
            if ($gameSystem.isInGacha(item)) {
                numLot = Number(item.meta.gachaNumLot || '1');
				if ($gameSystem.rateupArmors.contains(item.id)) {
					numLot *= 3;
				}
                for (j = 0; j < numLot; j++) {
                    this._lot.push(item);
    ...
    Scene_Gacha.prototype.lotrefresh = function (lot) {
		// Refresh Lot and Set Lot Type
     ...
     if (lottype == 'standard') {
		if (luck <= 0.15) {
			pullrank = 3; //Rank 3 Pull Rate
		} else if (luck <= 0.4) {
			pullrank = 2; //Rank 2 Pull Rate
		} else {
			pullrank = 1; //Rank 3 Pull Rate
		}	
		} else if (lottype == 'premium') {
		if (luck <= 0.01) {
			pullrank = 5; //Rank 5 Pull Rate
		} else if (luck <= 0.1) {
			pullrank = 4; //Rank 4 Pull Rate
		} else {
			pullrank = 3; //Rank 3 Pull Rate
		}
		} else {
        ...
        for (i = 1; i < $dataItems.length; i++) {
            item = $dataItems[i];
            var rank = Number(item.meta.gachaRank || '-1');
            if ($gameSystem.isInGacha(item) && (rank == pullrank)) {
                numLot = Number(item.meta.gachaNumLot || '1');
				if ($gameSystem.rateupItems.contains(item.id)) {
					numLot *= 3;
				}
    ...
    Scene_Gacha.prototype.createCommandWindow = function () {
        ...
        this._commandWindow.setHandler('gacha1', this.commandGacha.bind(this,'standard'));
        this._commandWindow.setHandler('gacha2', this.commandGacha.bind(this,'premium'));
        this._commandWindow.setHandler('gacha3', this.commandGacha.bind(this,'super'));
    ...
    Scene_Gacha.prototype.commandGacha = function (lot) {
        // Draw lots
		 var lottype = (lot);
		 
		 
		this.lotrefresh(lottype);
		
        if (this._lot.length <= 0) {
            this._item = null;
        }
        else {
			for (var i = 0; i < this._lot.length; i ++) {
				console.log('i: ', i, '; this._lot[i]:', this._lot[i], 'Lot: ',lottype);
			}
            ...
            this._goldWindow.refresh();
				...
				$gameParty.gainItem($dataItems[99], -3);
            this._goldWindow.refresh();
			} else {
				$gameParty.gainItem($dataItems[99], -30);
            this._goldWindow.refresh();
			}
        }
    ...
    Window_GachaCommand.prototype.makeCommandList = function () {
        this.addCommand('Gold', 'gacha1', $gameParty.gold() >= amount);
        this.addCommand('Premium', 'gacha2', $gameParty.numItems($dataItems[99]) >= 3);
        this.addCommand('Super', 'gacha3', $gameParty.numItems($dataItems[99]) >= 30);
