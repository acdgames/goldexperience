 ...
 *
 * @param srpgPredictionWindowMode
 * @desc 戦闘予測ウィンドウの表示を変更します。(1:フル / 2:攻撃名のみ / 3:表示N/A)
 * @default 1
 *
 * @param srpgAutoBattleStateId
...
//変数関係の処理
    ...
    Game_System.prototype.startSRPG = function() {
        ...
        this.clearData(); //Data initialization
        this.setAllEventType(); //Event type setting
        this.setSrpgActors(); //Creating actor data
        this.setSrpgEnemys(); //Creation of energy data
        $gameMap.setEventImages();   // Change event graphics to match unit data
        this.runBattleStartEvent(); // Execute Battle Start event
        $gameVariables.setValue(_turnVarID, 1); //Set Turn Variable to 1
        $gameSystem.resetSearchedItemList(); //Initialize searched coordinates
        $gameMap.events().forEach(function(event) {
            if (event.isType() === 'actor' || event.isType() === 'enemy') {
				evdata = $gameSystem.EventToUnit(event.eventId())[1];
				if (evdata) {
				$gameSystem.EventToUnit(event.eventId())[1].ongameStartEffects();
				};
			};
		});
    ...
    Game_System.prototype.ActorToEvent = function(actor_id) {
	    ...
	    Game_System.prototype.UnitToEvent = function(unit) {
        var eventId = 0;
        $gameMap.events().forEach(function(event) {
            if (event.isType() === 'actor' && unit.isActor()) {
                var testunit = $gameSystem.EventToUnit(event.eventId())[1];
				if (unit == testunit) {
					unitDat = $gameMap.event(event.eventId());
				};
            } else if (event.isType() === 'enemy' && unit.isEnemy()) {
                var testunit = $gameSystem.EventToUnit(event.eventId())[1];
				if (unit == testunit) {
					unitDat = $gameMap.event(event.eventId());
				};
	...
	Game_System.prototype.getSRPGenemies = function(unit) {
        var eventId = 0;
		var enemyarray = [];
        $gameMap.events().forEach(function(event) {
            if (event.isType() === 'enemy' && unit.isActor()) {
					enemyarray.push($gameMap.event(event.eventId()));
            } else if (event.isType() === 'actor' && unit.isEnemy()) {
					enemyarray.push($gameMap.event(event.eventId()));
			};
        });
        return enemyarray;
	...
	Game_System.prototype.getSRPGallies = function(unit) {
        var eventId = 0;
		var allyarray = [];
        $gameMap.events().forEach(function(event) {
            if (event.isType() === 'enemy' && unit.isEnemy()) {
					allyarray.push($gameMap.event(event.eventId()));
            } else if (event.isType() === 'actor' && unit.isActor()) {
					allyarray.push($gameMap.event(event.eventId()));
			};
        });
        return allyarray;
    ...
    // Read an actor from the event memo and associate it with the corresponding event ID
    Game_System.prototype.setSrpgActors = function() {
		...
		//(array[i] && actorClass == array[i].currentClass().name.toUpperCase())
        $gameMap.events().forEach(function(event) {
            if (event.isType() === 'actor') {
                var actorId = event.event().meta.id ? Number(event.event().meta.id) : 0;
                var actorClass = event.event().meta.class ? String(event.event().meta.class).toUpperCase() : 0;
                if (actorId > 0) {
                    var actor_unit = $gameActors.actor(actorId);
				} else if (actorClass !== 0) {
					for (var h=0; h < array.length; h++) {
						if (actorClass == array[h].currentClass().name.toUpperCase()) {
							actor_unit = array[h];
							array.splice(h,1);
							break;
						}
    ...
    Game_System.prototype.endSRPG = function() {
        ...
        $gameMap.events().forEach(function(event) {
            ...
            if (battlerArray && (battlerArray[0] === 'actor' || battlerArray[0] === 'enemy')) {
                ...
                battlerArray[1].ongameEndEffects();
            }
        });
        this._SRPGMode = false;
    ...
    Game_System.prototype.srpgStartActorTurn = function() {
        ...
        $gameMap.events().forEach(function(event) {
            ...
            if (battlerArray && battlerArray[0] === 'enemy' && battlerArray[1].isAlive()) {
                battlerArray[1].onphaseEndEffects();
				battlerArray[1].onTurnEnd();
        ...
        $gameMap.events().forEach(function(event) {
            ...
            var battlerArray = $gameSystem.EventToUnit(event.eventId());
				...
				battlerArray[1].onTurnEnd();
                battlerArray[1].SRPGActionTimesSet();
                battlerArray[1].onTurnStartStateEffects();
				}
            });
...
//====================================================================
    ...
    Game_Action.prototype.srpgPredictionDamage = function(target) {
        ...
        if (this.item().damage.type > 0) {
            var baseValue = this.makeDamageValue(target);
        } else {
            var baseValue = 0;
        }
        var value = baseValue;
...
//====================================================================
    ...
    Game_BattlerBase.prototype.srpgSkillRange = function(skill) {
	    ...
	    // スキル・アイテムの射程を返す（定義は、gameActor, gameEnemyで行う）
    Game_BattlerBase.prototype.srpgCounterRange = function(skill) {
        return 0;
    };
    ...
    Game_BattlerBase.prototype.canUse = function(item) {
        if ($gameSystem.isSRPGMode() == true) {
			...
			//Range Detection
			if (this._srpgActionTiming == 1) {
			//console.log("Base Range- Min: " + this.srpgSkillMinRange(item) + " Max: " + this.srpgSkillRange(item));
			//console.log("Counter Range- Min: " + this.srpgCounterMinRange(item) + " Max: " + this.srpgCounterRange(item));
			};
		if (this._srpgActionTiming == 1) {
            if (($gameSystem.isSubBattlePhase() === 'invoke_action' ||
                ...
                (this.srpgSkillRange(item) < $gameTemp.SrpgDistance() && this.srpgCounterRange(item) < $gameTemp.SrpgDistance()||
                this.srpgSkillMinRange(item) > $gameTemp.SrpgDistance() && this.srpgCounterMinRange(item) > $gameTemp.SrpgDistance()||
...
//====================================================================
    ...
    Game_Battler.prototype.onTurnEnd = function() {
        if ($gameSystem.isSRPGMode() == true) {
            this.regenerateAll();
			//console.log($gameSystem.isBattlePhase());
...
//====================================================================
	...
	//Actor Counter Range
	Game_Actor.prototype.srpgCounterRange = function(skill) {
        var range = 1;
        if (skill && skill.meta.srpgCounterRange == -1 || skill && !skill.meta.srpgCounterRange) {
            if (!this.hasNoWeapons()) {
                weapon = this.weapons()[0];
                range = (weapon.meta.srpgCounterRange||weapon.meta.weaponRange);
            }
        } else if (skill.meta.srpgCounterRange) {
            range = skill.meta.srpgCounterRange;
        } else {
            range = 1;
        }
		this.states().forEach(function(state) {
            if (state.meta.srpgCounterRange) {
                range += eval(state.meta.srpgCounterRange);
            }
        });
		range = Number(range).clamp(0,100);
        return Number(range);
    };
    ...
    Game_Actor.prototype.srpgCounterMinRange = function(skill) {
        var minRange = 0;
        if (skill) {
            if (skill.meta.srpgCounterMinRange == -1 || !skill.meta.srpgCounterMinRange) {
                if (!this.hasNoWeapons()) {
                    var weapon = this.weapons()[0];
                    minRange = (weapon.meta.srpgCounterMinRange || weapon.meta.weaponMinRange);
                }
            } else if (skill.meta.srpgCounterMinRange) {
                minRange = (skill.meta.srpgCounterMinRange || skill.meta.srpgMinRange);
            }
            if (!minRange) {
                minRange = 0;
            }
		...
		this.states().forEach(function(state) {
            if (state.meta.srpgCounterMinRange) {
                minRange += eval(state.meta.srpgCounterMinRange);
            }
        });
        //if (minRange > this.srpgSkillRange(skill)) {
        //    minRange = this.srpgSkillRange(skill);
        //}
		minRange = Number(minRange).clamp(0,100);
        return Number(minRange);
    };
...
//====================================================================
	...
	//Enemy Counter Range
	Game_Enemy.prototype.srpgCounterRange = function(skill) {
        var range = 1;
		if (skill) {
        if (skill.meta.srpgCounterRange == -1 || skill && !skill.meta.srpgCounterRange) {
            if (!this.hasNoWeapons()) {
                weapon = $dataWeapons[this.enemy().meta.srpgWeapon];
                range = (weapon.meta.srpgCounterRange||weapon.meta.weaponRange);
            } else {
                range = (this.enemy().meta.srpgCounterRange || 1);
            }
        } else if (skill.meta.srpgCounterRange) {
            range = skill.meta.srpgCounterRange;
        } else {
	...
	Game_Enemy.prototype.srpgCounterMinRange = function(skill) {
        var minRange = 0;
        if (skill) {
            if (skill.meta.srpgCounterMinRange == -1 || !skill.meta.srpgCounterMinRange) {
                if (!this.hasNoWeapons()) {
					...
					minRange = (this.enemy().meta.srpgCounterMinRange || 0);
				};
            } else if (skill.meta.srpgCounterMinRange) {
                minRange = (skill.meta.srpgCounterMinRange || skill.meta.srpgMinRange);
            }
...
//====================================================================
    ...
    Window_Base.prototype.drawEnemyClass = function(enemy, x, y, width) {
        ...
        this.resetTextColor();
		if (enemy.isStateAffected(515)) {
			this.changeTextColor(this.textColor(2));
		} else if (enemy.isStateAffected(516)) {
			this.changeTextColor(this.textColor(4));
		} else if (enemy.isStateAffected(517)) {
			this.changeTextColor(this.textColor(3));
		} else if (enemy.isStateAffected(518)) {
	...
	//Actor Class
    Window_Base.prototype.drawSRPGActorClass = function(actor, x, y, width) {
        width = width || 168;
        this.resetTextColor();
	...
	//Enemy Skills
    Window_Base.prototype.drawEnemySkills = function(enemy, x, y, line) {
        width = 360;
        var AS = enemy.enemy().meta.HelpAS || 'N/A';
        var ST = enemy.enemy().meta.HelpST || 'N/A';
        var TC = enemy.enemy().meta.HelpTC || 'N/A';
        var AU = enemy.enemy().meta.HelpAU || 'N/A';
        var SP = enemy.enemy().meta.HelpSP || 'N/A';
        this.changeTextColor(this.systemColor());
        this.drawText('AS: ', x, y, width);
        this.drawText('Stance: ', x, y+line, width);
        this.drawText('Technique: ', x, y+(line*2), width);
        this.drawText('Aura: ', x, y+(line*3), width);
        this.drawText('Special: ', x, y+(line*4), width);
        this.resetTextColor();
        this.drawText(AS, x+45, y, width-45);
        this.drawText(ST, x+100, y+line, width-100);
        this.drawText(TC, x+140, y+(line*2), width-140);
        this.drawText(AU, x+75, y+(line*3), width-75);
        this.drawText(SP, x+120, y+(line*4), width-120);
    };
...
//====================================================================
// ●Window_SrpgStatus THE FUCKING STATUS WINDOW
//====================================================================
    ...
    Window_SrpgStatus.prototype.windowHeight = function() {
        ...
        return this.fittingHeight(this._full ? 15 : 10);
		} else {
    ...
    Window_SrpgStatus.prototype.drawContentsActor = function() {
        ...
        this.drawSRPGActorClass(this._battler, 192, lineHeight * 0);
    ...
    Window_SrpgStatus.prototype.drawContentsEnemy = function() {
        ...
        this.drawParameters(6, lineHeight * 6);
        this.drawSrpgParameters(6, lineHeight * 9);
        this.drawEnemySkills(this._battler, 6, lineHeight * 10, lineHeight);
    };
...
//====================================================================
    ...
    Window_SrpgActorCommandStatus.prototype.drawContents = function() {
        ...
        this.drawSRPGActorClass(this._battler, x2, y);
...
//====================================================================
    ...
    Window_SrpgBattleStatus.prototype.drawBasicInfoEnemy = function(x, y) {
	    ...
	    Window_SrpgBattleStatus.prototype.drawActorHp = function(actor, wx, wy, ww) {
    ww = ww || 186;
    var color1 = this.hpGaugeColor1();
    var color2 = this.hpGaugeColor2();
    ...
    Window_SrpgBattleStatus.prototype.drawActorHpbak = function(actor, x, y, width) {
        width = width || 186;
        var color1 = this.hpGaugeColor1();
        var color2 = this.hpGaugeColor2();
...
//====================================================================
    ...
    Window_SrpgBattle.prototype.makeCommandList = function() {
        this.addCommand('Engage', 'battleStart', this.isEnabled(this._item));
...
//====================================================================
    ...
    Scene_Map.prototype.createSrpgStatusWindow = function() {
		...
		this._mapSrpgStatusWindow._full = true;
		//396 standard height
    ...
    Scene_Map.prototype.createSrpgPredictionWindow = function() {
		this._mapSrpgStatusWindow.refresh();
        this._mapSrpgPredictionWindow = new Window_SrpgPrediction(0, 0);
        this._mapSrpgPredictionWindow.y = 396;
    ...
    Scene_Map.prototype.createSrpgBattleWindow = function() {
        ...
        this._mapSrpgBattleWindow.y = 396 + this._mapSrpgPredictionWindow.windowHeight();
    ...
    Scene_Map.prototype.update = function() {
        ...
        if (flag[0]) {
            if (!this._mapSrpgStatusWindow.isOpen() && !this._mapSrpgStatusWindow.isOpening()) {
				//this._mapSrpgStatusWindow._full = false;
        ...
        if (flag[0]) {
            ...
            if (!this._mapSrpgTargetWindow.isOpen() && !this._mapSrpgTargetWindow.isOpening()) {
				//Shortens Enemy Window
				this._mapSrpgTargetWindow._full = false;
				this._mapSrpgStatusWindow._full = false;
				this._mapSrpgStatusWindow.refresh();
    ...
    Scene_Map.prototype.srpgInvokeEnemyCommand = function() {
        ...
        if (_srpgStandUnitSkip === 'true' && enemy.battleMode() === 'stand') {
			...
			//Create list of viable targets for actions
            $gameTemp.clearMoveTable();
            if (canAttackTargets.length === 0) {
                $gameTemp.setActiveEvent(event);
    ...
    Scene_Map.prototype.srpgInvokeEnemyMove = function() {
        ...
        $gameSystem.srpgMakeMoveTable(event);
        this.srpgPriorityTarget(enemy); //Set Prefered Targets
        var canAttackTargets = this.srpgMakeCanAttackTargets(enemy, targetType); 
...
//====================================================================
    ...
    BattleManager.startBattle = function() {
        if ($gameSystem.isSRPGMode() == true) {
			...
			if ($gameParty.battleMembers()[0] !== undefined && $gameTroop._enemies[0] !== undefined) {
			$gameParty.battleMembers()[0].oncombatStartEffects();
			$gameTroop._enemies[0].oncombatStartEffects();
			};
        } else {
    ...
    BattleManager.endBattle = function(result) {
		...
		if ($gameParty.battleMembers()[0] && $gameTroop._enemies[0]) {
			//console.log($gameParty.battleMembers()[0]);
			//console.log($gameTroop._enemies[0]);
        $gameParty.battleMembers()[0].onAfterBattleEffects();
		$gameTroop._enemies[0].onAfterBattleEffects();
        $gameParty.battleMembers()[0].setActionTiming(-1);
		$gameTroop._enemies[0].setActionTiming(-1);
		$gameParty.battleMembers()[0].clearResult();
		$gameTroop._enemies[0].clearResult();
		};
