...
Game_Battler.prototype.lmbsActionUserClear = function() {
	...
	var comboSkillID = this._lmbs_ActionUser[13];
	...
	if (!this.lmbsIsKnockbacking() && comboSkillID && comboSkillID != 0 && Input.isPressed(Moghunter.lmbsKeyAttack)) {
		this._lmbs_wait = 0;
		this._lmbs_ForcedAction = true;
		this._lmbs_AerialActionDone = false;
		this.lmbsAction(comboSkillID,type)
	} else {
		this.lmbsSetTarget();
	};
};
...
Game_Battler.prototype.lmbsAction = function(actionID,type,target) {
    ...
    item_notes.forEach(function(note) {
		 ...
		 if (note_data[0].toLowerCase() == "lmbs combo chain"){			 
             var par = note_data[1].split(':');
			 comboLink = Number(par[0]);
         };
		 if (note_data[0].toLowerCase() == "lmbs animation id"){			 
             var par = note_data[1].split(':');
	...
	this._lmbs_ActionUser = [item,pose,poseDuration,userMovement,skillLink,superGuard,skillType,aerialMode,poseLoop,poseSpeed,normalAttack,type,userInvulnerable,comboLink];
