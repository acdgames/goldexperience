...
(function() {
  ...
  Game_Bullet.prototype.update = function() {
    if ($gameTemp.isStopMapEventMovement()) {
	} else {
    this._x += this._vx;
    this._y += this._vy;
    this._count--;
  ...
  Game_Bullet.prototype.collideCharacter = function(character) {
    ...
    return this._x - this._collideSize <= (character._realX + character._offsetX) + character._collideW &&
           this._x + this._collideSize >= (character._realX + character._offsetX) - character._collideW &&
           this._y - this._collideSize <= (character._realY + character._offsetY)  &&
           this._y + this._collideSize >= (character._realY + character._offsetY) - character._collideH;
  };
  ...
  //Non-Meta Player Variables Init
  ...
  Game_CharacterBase.prototype.update = function() {
	...
	if (this._antigravity > 0) this._antigravity--;
	if (this._jumpFrame > 0) this._jumpFrame--;
  };
  ...
  Game_CharacterBase.prototype.updateGravity = function() {
	...
	if (this._antigravity == 0) {
    this._vy = Math.min(this._vy + this._gravity, this.maxFallSpeed());
	} else {
		this._vy = Math.min(this._vy + this._gravity, 0);
	}
  };
  ...
  Game_CharacterBase.prototype.updateMoveCount = function() {
    ...
    if (this._moveCount == 0 && !this.isDashing()) {
      this._vx = 0;
      if (this._gravity == 0 || this._antigravity) this._vy = 0;
    }
  };
  ...
  Game_CharacterBase.prototype.collideCharacterUp = function() {
    ...
    for (var i = 0; i < targets.length; i++) {
      ...
      if (this.isCollide(character) && !character.isCarried()) {
		...
		//Contact Damage
		if (character.eventId() > 0) {
			if (character._contactDamage > 0) {
				$gameParty.members()[0].gainHp(-character._contactDamage);
			}
		}
  ...
  Game_CharacterBase.prototype.isCollide = function(character) {
    ...
    return this._realX - this._collideW <= (character._realX + character._offsetX) + character._collideW &&
           this._realX + this._collideW >= (character._realX + character._offsetX) - character._collideW &&
           this._realY - this._collideH <= (character._realY + character._offsetY) &&
           this._realY >= (character._realY + character._offsetY) - character._collideH;
  };
  ...
  Game_Character.prototype.moveRandom = function() {
    if (this._gravity == 0 || this._antigravity > 0) {
  ...
  Game_Player.prototype.refresh = function() {
    ...
    if (actor) {
      ...
      this._offsetX       = +(data.meta['offX'] || 0);
      this._offsetY       = +(data.meta['offY'] || 0);
      this._fallGuard = +(data.meta['fall_guard'] || 0);
      this._guardSpeed = +(data.meta['guard_speed'] || 0);
      this._invincibleTime = +(data.meta['invincible_time'] || 0);
  ...
  Game_Event.prototype.initMembers = function() {
    ...
    this._contactDamage = 0;
    this._deadCommonEvent = null;
    this._commentParams = {};
  };
...
//      if (this.isTriggerIn([1, 2]) && this.isNormalPriority() === normal) {
  ...
  Game_Event.prototype.setupPage = function() {
    ...
    if (this._pageIndex >= 0) {
      ...
      this._offsetX       = +this.loadTagParam('offX') || 0;
      this._offsetY       = +this.loadTagParam('offY') || 0;
      this._weight         = +this.loadTagParam('weight') || 0;
      this._deadSelfSwitch = this.loadTagParam('dead');
      this._contactDamage = this.loadTagParam('contactDamage') || 0;
      this._deadCommonEvent = this.loadTagParam('deadCommon');
  ...
  Game_Event.prototype.updateLock = function() {
    this._lockCount--;
      if (this.battler().isDead()) {
		if (this._deadCommonEvent !== null) {
			var ev = this._deadCommonEvent;
			console.log(ev);
			$gameTemp.reserveCommonEvent(ev);
			this._deadCommonEvent = null;
		}
	  }
