...
(function() {
  ...
  Game_Player.prototype.mapEdgeTransferUpdate = function() {
	 var chkx = this._realX;
	 var chky = this._realY;
    if (chkx <= 0.5 && $dataMap.meta.tfrleft) {
      this.mapEdgeTransfer($dataMap.meta.tfrleft);
    }
    if (chkx >= (($dataMap.width - 1)+0.5) && $dataMap.meta.tfrright) {
	//console.log(this._vx);
      this.mapEdgeTransfer($dataMap.meta.tfrright);
    }
    if (chky <= 0.5 && $dataMap.meta.tfrup) {
      this.mapEdgeTransfer($dataMap.meta.tfrup);
    }
    if (chky >= (($dataMap.height - 1)+0.5) && $dataMap.meta.tfrdown) {
