...
(function(){
    ...
    Game_CharacterBase.prototype.makeMoveTable = function(x, y, move, route) {
        ...
        if (route[route.length - 1] != 2) {
            if (this.srpgMoveCanPass(x, y, 8)) {
                var cost = this.getMoveCost(x, y - 1);
                if ($gameTemp.MoveTable(x, y - 1)[0] < move - cost) {
                    if ($gameTemp.MoveTable(x, y - 1)[0] < 0) {
                        $gameTemp.pushMoveList([x, y - 1, false]);
                    }
                    $gameTemp.setMoveTable(x, y - 1, move - cost, route.concat(8));
                    this.makeMoveTable(x, y - 1, move - cost, route.concat(8));
                }
            }
        }
        if (route[route.length - 1] != 4) {
            if (this.srpgMoveCanPass(x, y, 6)) {
                var cost = this.getMoveCost(x + 1, y);
                if ($gameTemp.MoveTable(x + 1, y)[0] < move - cost) {
                    if ($gameTemp.MoveTable(x + 1, y)[0] < 0) {
                        $gameTemp.pushMoveList([x + 1, y, false]);
                    }
                    $gameTemp.setMoveTable(x + 1, y, move - cost, route.concat(6));
                    this.makeMoveTable(x + 1, y, move - cost, route.concat(6));
                }
            }
        }
        if (route[route.length - 1] != 6) {
            if (this.srpgMoveCanPass(x, y, 4)) {
                var cost = this.getMoveCost(x - 1, y);
                if ($gameTemp.MoveTable(x - 1, y)[0] < move - cost) {
                    if ($gameTemp.MoveTable(x - 1, y)[0] < 0) {
                        $gameTemp.pushMoveList([x - 1, y, false]);
                    }
                    $gameTemp.setMoveTable(x - 1, y, move - cost, route.concat(4));
                    this.makeMoveTable(x - 1, y, move - cost, route.concat(4));
                }
            }
        }
        if (route[route.length - 1] != 8) {
            if (this.srpgMoveCanPass(x, y, 2)) {
                var cost = this.getMoveCost(x, y + 1);
                if ($gameTemp.MoveTable(x, y + 1)[0] < move - cost) {
                    if ($gameTemp.MoveTable(x, y + 1)[0] < 0) {
                        $gameTemp.pushMoveList([x, y + 1, false]);
                    }
                    $gameTemp.setMoveTable(x, y + 1, move - cost, route.concat(2));
                    this.makeMoveTable(x, y + 1, move - cost, route.concat(2));
                }
            }
        }
    };
    Game_CharacterBase.prototype.getMoveCost = function(x, y) {
        var cost = MovingCostList[$gameMap.terrainTag(x, y)];
        if (this.isEvent()) {
            var battlerArray = $gameSystem.EventToUnit(this._eventId);
            var unitdata;
            if (this.isType() === 'actor') {
                unitdata = battlerArray[1].currentClass();
            }
            if (this.isType() === 'enemy') {
                unitdata = battlerArray[1].enemy();
            }
            if (eval('unitdata.meta.srpgTE_Tag' + $gameMap.terrainTag(x, y) + 'MC')) {
                cost = Number(eval('unitdata.meta.srpgTE_Tag' + $gameMap.terrainTag(x, y) + 'MC'));
            }
        }
