 ...
 * movement.
 * Stealth gauge (on or off) - Turns the on-screen gauge on or off
 * Stealth reset - Resets the detection state for all events
 *
 * ============================================================================
 * Notetags
 * ============================================================================
 ...
 * 
 * Event Notetag:
 *   <Patrol: 1 Brightness: 100 Loudness: 125 Detection: 0 BypassSwitch: 3>
 ...
 *	 BypassSwitch is optional.  If it is set and the appropriate switch is ON, the
 *	 event will not find the player.  Nor will it have a Stealth score.
...
//=============================================================================
// Globals - THESE ARE NOT SAVED
//=============================================================================

var $stealthWindow=null;
var $stealth=null;
...
//=============================================================================
Stealth.prototype.initialize = function() {
	...
	*/
	...
	gauge_setting=WS_StealthConfig.Param.stealthGauge;
	if (!gauge_setting)
	{
		this.stealth_gauge=$gameSwitches.getValue(-999);
	}
	else
	{
		gauge_setting=gauge_setting.toLowerCase();
		if (gauge_setting == "on" || gauge_setting == "true" || gauge_setting == "yes") {
			console.log("Stealth gauge is enabled");
			this.stealth_gauge=true;
		}
		else
		{
			console.log("Stealth gauge is disabled");
			this.stealth_gauge=false;
		}
	}
	$gameSwitches.setValue(-999, this.stealth_gauge);
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	...
	command_lower=command.toLowerCase();
	if (command_lower === 'stealth' && args.length > 0) {
		...
		case 'noise':
			...
			$stealth.checkAllEvents(parseInt(args[1]), parseInt(args[2]));
			$stealth.updateGauge();
			break;
		/* Resets the stealth state of all of the events */
		case 'reset':
			console.log("Resetting all events");
			$stealth.resetEvents();
			$stealth.updateGauge();
			
			/* We aren't moving right now so there's no noise to check */
			$stealth.checkAllEvents(0, 100);
			$stealth.updateGauge();
			break;
		case 'gauge':
			if (args[1].toLowerCase() == "on")
			{
				console.log("Turning onscreen gauge on");
				$stealth.setGauge(true);
				$stealth.updateGauge();
			}
			else
			{
				console.log("Turning onscreen gauge off");
				$stealth.setGauge(false);
				$stealth.updateGauge();
			}
		break;
		}
...
//=============================================================================
// Resets the states of all of the events
//=============================================================================
Stealth.prototype.resetEvents = function()
{
		for (eventLoop=0; eventLoop<$stealth.events.length; eventLoop++) {
			event=$stealth.events[eventLoop];
			$stealth.resetEvent(event);
		}
}

//=============================================================================
// Resets the states of a specific event
//=============================================================================
Stealth.prototype.resetEvent = function(event)
{
	console.log("Resetting an event");
	event.search = {};
	event.search["state"]=1;
	event.search["cooldown"]=0;
	var key = [$gameMap.mapId(), event.eventId(), 'B'];
	$gameSelfSwitches.setValue(key, false);
	key=[$gameMap.mapId(), event.eventId(), 'C'];;
	$gameSelfSwitches.setValue(key, false);
}
...
Stealth.prototype.searchBase = function(source, targets, base_signals, radius, modifier_function, check_function, x, y) {
	...
	/* Check for a bypass switch */
	if (event_meta && event_meta.BypassSwitch) {
		switch_id=parseInt(event_meta.BypassSwitch);
...
//=============================================================================
// Turns the on-screen gauge on or off
//=============================================================================
Stealth.prototype.setGauge = function(enabled)
{
	$stealth.stealth_gauge=enabled;
	/* Store this for saving */
	$gameSwitches.setValue(-999, enabled);
}
...
Stealth.prototype.updateDetectionStatus = function(event, result)
{
	...
	if (result)
	{
		...
		else if (current_score >=  WS_StealthConfig.Param.searchThreshold)
		{
			...
			var key = [$gameMap.mapId(), event.eventId(), 'B'];
			$gameSelfSwitches.setValue(key, true);
			console.log('Current Data1: ' + key);
			key=[$gameMap.mapId(), event.eventId(), 'C'];
			$gameSelfSwitches.setValue(key, false);
			console.log('Current Data2: ' + key);
			return;
		}
	}
...
Stealth.prototype.getEventMeta = function(event) 
{
	if (!event)
		return null;
	if (!$dataMap.events[event.eventId()])
		return null;
...
{
	if (!event || !event.eventId || !$dataMap[events.eventId()])
		return false;
...
Window_Stealth.prototype.setGauge = function(value)
{
	...
	if (value > this.maxValue)
		value=this.maxValue;
	if ($stealth && !$stealth.stealth_gauge)
		return;
...
Stealth.prototype.updateGauge = function() {
	...
	if (this.stealth_gauge)
	{
		...
		{
			$stealthWindow.show();
			$stealthWindow.setGauge(drawScore);
		}
	}
	else
	{
...
Stealth.prototype.isPatrolling = function(event_id) {
	if (this.eventPatrols[event_id] && this.eventPatrols[event_id].isActive())
	{
		...
		var key = [$gameMap.mapId(), event_id, 'B'];
		if ($gameSelfSwitches.value(key))
		{
			return false;
		}
...
Scene_Map.prototype.onMapLoaded = function() {
	...
	$stealthWindow=new Window_Stealth();
	this.addWindow($stealthWindow);
	$stealthWindow.refresh();
	$stealthWindow.activate();
	if ($stealth)
		$stealth.updateGauge();
}
...
Game_Actor.prototype.stealthModifiers = function() {
	...
	if (!$stealth.actor_cache[this.actorId()]) {
		meta=$dataActors[this.actorId()].meta;
		$stealth.actor_cache[this.actorId()]=new StealthModifier(meta);
	}
	modifier_list.push($stealth.actor_cache[this.actorId()]);
	
	// The current class
	if (this._classId)
	{
		if (!$stealth.class_cache[this._classId])
		{
			meta=$dataClasses[this._classId].meta;
			$stealth.class_cache[this._classId] = 
			new StealthModifier(meta);
		}
	}
	
	modifier=$stealth.class_cache[this._classId];
	if (modifier)
	...
	for (mod=0; mod<items.length; mod++)
	{
		...
		if (current === null)
			continue;
		if ($stealth.item_cache[current.id] === undefined)
		{
			meta={};
			...
			$stealth.item_cache[current.id]=new StealthModifier(meta);
		}
		modifier=$stealth.item_cache[current.id];
		modifier_list.push(modifier);
	}
	...
	for (mod=0; mod<states.length; mod++)
	{
		...
		if (current === null)
			continue;
		if (!$stealth.state_cache[current.id])
		{
			$stealth.state_cache[current.id]=new StealthModifier($dataStates[current.id].meta);
		}
		modifier=$stealth.state_cache[current.id];
		modifier_list.push(modifier);
	}
...
Game_CharacterBase.prototype.moveDiagonally = function(horz, vert) {
	...
	{
		// Trigger every OTHER event to do the search
		$stealth.checkAllEvents(100+(100*$gamePlayer.isDashing()), 100);
		$stealth.updateGauge();
	}
}
//=============================================================================
...
Game_Event.prototype.updateSelfMovement = function() {
	...
	if ($stealth.isPatrolling(this.eventId()))
	{
		patrol_object=$stealth.eventPatrols[this.eventId()];
		if (patrol_object)
		{
			patrol_object.move();
		}
...
//=============================================================================
// Remove the stealth window when we save
//=============================================================================

/*
var WS_Stealth_DM_makeSaveContents = DataManager.makeSaveContents;
DataManager.makeSaveContents = function() {
	return WS_Stealth_DM_makeSaveContents.call(this, arguments);
}
*/

//=============================================================================
// Load the stealth window when we load
//=============================================================================
/*
var WS_Stealth_DM_extractSaveContents = DataManager.extractSaveContents;
 DataManager.extractSaveContents = function() {
	 WS_Stealth_DM_extractSaveContents.call(this, arguments);
 }
*/
})();
//=============================================================================
